    ...
    <div :style="isDragging ? 'user-select: none;' : ''">
      <div class="fixed inset-0 min-h-full"
        ...
        <div class="absolute bottom-0 left-0 mb-2 mx-4 right-0">
          <Slider v-model="axes.wght" @input="sliderChange" :min="0" :max="1000" :markers="{0: 'Light', 400: 'Normal', 600: 'Bold', 1000: 'Black'}" :background="secondaryColor" :color="primaryColor"  @start="dragStart" @end="dragEnd" :globalDragging="isDragging" />
      ...
      <div class="bg-beige bottom-0 fixed overflow-y-auto pb-4 pt-1 right-0 top-0 w-full lg:w-1/4" v-show="isCartShown">
        ...
        <div class="flex items-center px-4 pt-4" v-if="total > 0">
          ...
          <button @click="formSubmit" class="inline-block h-10 leading-10 rounded-full px-4" :class="[!total ? 'bg-gray-300' : '']" :style="[ total ? {color: primaryColor, background: secondaryColor} : {} ]">Buy</button>
        </div>
      </div>
    </div>
  </div>
